        ...
        <section className="rounded-3xl border border-slate-300 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-4">
            ...
            <button
              ...
              onClick={demo.listening ? demo.stopListening : demo.sendInterrupt}
              disabled={!demo.connected}
            >
              {demo.listening ? "Stop Listening" : "Interrupt"}
            </button>
          </div>
          ...
          <p className="mt-3 text-sm text-slate-600">
            Session ID:{" "}
            <span className="font-mono text-slate-900">{demo.sessionId ?? "not started"}</span>
          </p>
          {demo.error ? (
          ...
          <div className="space-y-5">
            <section className="rounded-3xl border border-slate-300 bg-white p-5">
              ...
              <p className="mt-2 text-sm text-slate-600">
                Current state: <span className="font-mono">{demo.currentState}</span>
              </p>
            ...
            <section className="rounded-3xl border border-slate-300 bg-white p-5">
              ...
              <p className="mt-3 text-sm text-slate-600">
                ...
                <span className="font-medium">
                  {finalConfirmation?.required ? "yes" : "no"}
                </span>
              </p>
              <p className="mt-1 text-sm text-slate-600">
                Final confirmation approved:{" "}
                <span className="font-medium">
                  {finalConfirmation?.confirmed ? "yes" : "no"}
                </span>
              </p>
              {demo.finalConfirmationPending ? (
          ...
          <article className="rounded-3xl border border-slate-300 bg-white p-5">
            ...
            {demo.runtimeScreenshot?.image_base64 ? (
              ...
              <p className="mt-3 rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-4 py-6 text-sm text-slate-500">
                No runtime screenshot available yet.
              </p>
            )}
            <div className="mt-3 space-y-1 text-sm text-slate-700">
